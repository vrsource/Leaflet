...
L.extend(L.GeoJSON, {
	...
	geometryToLayer: function (geojson, pointToLayer, coordsToLatLng, vectorOptions, feature) {
		...
		case 'GeometryCollection':
			for (i = 0, len = geometry.geometries.length; i < len; i++) {
				...
				}, pointToLayer, coordsToLatLng, vectorOptions, feature));
			}
			return new L.FeatureGroup(layers);
...
L.DomEvent = {
	...
	shouldIgnoreEvents: function (el) {
		var className;
		while (el.parentNode) {
			className = el.parentNode.className;
			if (className && className.indexOf && className.indexOf('_leaflet_ev_ignore') >= 0) {
				return true;
			}
			el = el.parentNode;
		}
